...
import type { DeviceSummary, EventRow, EventType } from '../../domain/entities';
...
export function createHttpServer(deps: {
  config: { httpPort: number; apiKey?: string };
  eventBus: EventBus;
  sampleBus: EventBus;
  ...
  history(device: string, sinceMs: number): Promise<{ lqi: number; ts: string }[]>;
  meshHistory(sinceMs: number): Promise<{ ts: string; lqi: number; n: number }[]>;
  listEvents(opts: { type?: EventType; sinceMs?: number; limit?: number }): Promise<EventRow[]>;
  getLatestSnapshot(): Promise<{ ts: string; raw_json: string } | null>;
  triggerManualRefresh(): { ok: boolean; error?: string; promise?: Promise<{ ok: boolean; error?: string }> };
  getHealthStatus(): { mqttConnected: boolean; lastSampleAt: Date | null; lastSnapshotAt: Date | null };
}): { start(): void } {
  return {
    start(): void {
      const { config, eventBus, sampleBus } = deps;
      const app = express();
      ...
      app.get('/api/devices', async (_req, res) => {
        const [devices, aliases] = await Promise.all([deps.computeDeviceSummaries(), deps.getAllAliases()]);
        res.json({ devices: devices.map((d) => ({ ...d, alias: aliases.get(d.name) ?? null })) });
      });

      app.put('/api/devices/:name/alias', async (req, res) => {
        const name = req.params.name;
        if (!(await deps.listDeviceNames()).includes(name)) {
        ...
        await deps.setAlias(name, alias || null);
      ...
      app.get('/api/devices/:name/history', async (req, res) => {
        ...
        const points = (await deps.history(name, ms)).map((r) => ({ ts: r.ts, lqi: r.lqi }));
      ...
      app.get('/api/mesh/history', async (req, res) => {
        ...
        res.json({ range: queryStr(req, 'range'), points: await deps.meshHistory(ms) });
      });
      ...
      app.get('/api/events', async (req, res) => {
        ...
        res.json({
          events: await deps.listEvents({ type: validatedType, sinceMs: parseSince(queryStr(req, 'since')), limit }),
        });
      });
      ...
      app.get('/api/network/latest', async (_req, res) => {
        const row = await deps.getLatestSnapshot();
        if (!row) {
      ...
      app.post('/api/network/refresh', (req, res) => {
        const gate = deps.triggerManualRefresh();
        if (!gate.ok) {
      ...
      app.get('/api/health', (_req, res) => {
        ...
        res.json({
          ...
          lastSampleAt: status.lastSampleAt?.toISOString() ?? null,
