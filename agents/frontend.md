...
- Events are real-time via SSE (`/api/events/stream` → `connectEventStream()` → `reloadEventsForView()`); map view stays poll/manual by design.
- LQI charts are real-time via SSE (`/api/samples/stream` → `connectSampleStream()` → `poll()`).
